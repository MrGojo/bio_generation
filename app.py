...
# Define predefined bios based on career, personality, interests, and relationship goals
predefined_bios = {
...
def get_predefined_bio(career, personality, interests, relationship_goals):
    ...
    return predefined_bios.get((career, personality, interests, relationship_goals),
                               "Sorry, we couldn't find a predefined bio for you. Please try again.")

# Function to generate a bio using OpenAI
def generate_bio_with_openai(career, personality, interests, relationship_goals):
    try:
        # Call OpenAI to generate a dynamic bio
        response = openai.Completion.create(
            engine="text-davinci-003",  # Change this to the model you're using
        ...
        return bio, None  # Return the bio and no error message
    except openai.error.RateLimitError:
        # If OpenAI API exceeds the limit, use predefined bio
        return get_predefined_bio(career, personality, interests, relationship_goals), \
               "Oops! We've hit our daily limit for AI-generated bios. But don’t worry, here’s a bio based on your selections!"
    except openai.error.OpenAIError as e:
        # For other OpenAI errors, handle gracefully
        return get_predefined_bio(career, personality, interests, relationship_goals), \
               "There was an issue with generating your bio using AI right now "
...
def generate_bio():
    ...
    # Attempt to generate a bio using OpenAI, with fallback on error
    bio, error_message = generate_bio_with_openai(career, personality, interests, relationship_goals)
